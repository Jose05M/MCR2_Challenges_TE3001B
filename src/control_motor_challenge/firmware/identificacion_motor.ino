...
{  
  ...
  setPoint_RPM = constrain(msg->data, MSG_MIN_VAL, MSG_MAX_VAL);
  mag = abs(setPoint_RPM);
}
...
// Referencia
float setPoint_percent = 0.0;
//float setPoint_RPM = 0.0;
// Variable de process variable
float processVariable_percent = 0.0;

// Control
int pwm = 0;
  
const float alphaRPM = 0.20; 
const float SP_DEADBAND_PERCENT = 0.20;
...
void control_function(){
  ...
  rpm = rpm_filt/RPM_MAX;

  //CONVERTIR A PWM
  uint32_t duty = (uint32_t)(mag * PWM_MAX);
  ...
  // publicar datos
  msg_pub.data.data[0] = time_sec;
  msg_pub.data.data[1] = setPoint_RPM;
  msg_pub.data.data[2] = rpm * dir;
...
void setup() {
  ...
  msg_pub.data.data = (float*) malloc(3*sizeof(float));
  msg_pub.data.size = 3;
  msg_pub.data.capacity = 3;
}
